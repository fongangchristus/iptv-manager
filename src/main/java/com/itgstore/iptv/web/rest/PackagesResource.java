...
/**
 * REST controller for managing {@link com.itgstore.iptv.domain.Packages}.
 */
@RestController
@RequestMapping("/api")
public class PackagesResource {
     ...
     */
    @GetMapping("/packages")
    public ResponseEntity<List<PackagesDTO>> getAllPackages(Pageable pageable, @RequestParam(required = false, defaultValue = "false") boolean eagerload) {
        log.debug("REST request to get a page of Packages");
        Page<PackagesDTO> page;
        if (eagerload) {
            page = packagesService.findAllWithEagerRelationships(pageable);
        } else {
            page = packagesService.findAll(pageable);
        }
